...
public class ShowTemperature extends Thread {
    ...
    ArrayList<Double> getTemperatureByTime(int time) throws SQLException {
        temperature.clear();
        try {
...
//        sql = "select * from sample order by sample_time desc limit " + time;
    ...
    public double getAverageTemperatureBy60() throws SQLException {
        ...
        if (results == null) {
            System.out.println("不足六条，无法显示平均温度");
            return -2;
        }
        for (double data :
                results) {
        ...
        System.out.println("一分钟平均温度是：" + result);
        return result;
    }
    ...
    public void run() {
        ...
        while (true) {
            try {
                System.out.println("温度是：" + this.getTemperatureByTime(1).get(0).doubleValue());
                k++;
                sleep(10000);
